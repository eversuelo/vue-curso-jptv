...
const totalPagar = computed(() => {
    const total = props.carrito.reduce((acc, guitarra) => acc + (guitarra.precio * guitarra.cantidad), 0);
    // Formatter Money
    return new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(total);
})
defineEmits(['incrementar-cantidad', 'decrementar-cantidad', 'eliminar-guitarra', 'agregar-carrito','vaciar-carrito'])
</script>
<template>
        ...
        <div class="container-xl">
                                        ...
                                        <tr v-for="producto in carrito">
                                            ...
                                            <td class="flex align-items-start gap-4">
                                                <button type="button" class="btn btn-dark"
                                                    v-on:click="$emit('decrementar-cantidad', producto)">
                                                    -
                                                </button>
                                                {{ producto.cantidad }}
                                                <button type="button" class="btn btn-dark"
                                            ...
                                            <td>
                                                <button class="btn btn-danger" @click="$emit('eliminar-guitarra', producto)"
                                                    type="button">
                                                    X
                                                </button>
                                            </td>
            ...
            <div class="row mt-5">
                <div class="col-md-6 text-center text-md-start pt-5">
                    ...
                    <button type="button" @click="$emit('agregar-carrito', guitarra)"
